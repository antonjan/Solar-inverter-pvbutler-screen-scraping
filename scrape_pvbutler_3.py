...
inverter_data = json_cfg['json_object']
...
while True:
	try:
		...
		while True:
			...
			print("waiting for 35s")
			time.sleep(35)  # Makes Python wait for 5 seconds
			print("done Waiting")
				...
				if loop == 0:
					if html_content2 == "--":
						...
						#print("html tag value",html_content2)
									#time Date format "2022-03-11T00:00:00Z"
				loop += 1

			# Convert array to JSON object
			current_utc_time = datetime.utcnow()
			gmt_plus_2 = current_utc_time + timedelta(hours=2)
			formatted_time = gmt_plus_2.strftime("%Y-%m-%dT%H:%M:%S")
			print("Creating date")
			inverter_data[30][1] = formatted_time
